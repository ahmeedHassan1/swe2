...
function AdminPayroll() {
	...
	const fetchPayroll = async () => {
		const res = await fetchWithAuth(
			`/payroll/period?month=${month}&year=${year}`
		);
	...
	const openProcessDialog = async () => {
		...
		emps.forEach((emp) => {
			initialData[emp.id] = {
				...
				bonuses: 0,
				deductions: applyTax ? emp.salary * 0.1 : 0
			};
		});
	...
	const updatePayrollField = (empId, field, value) => {
		...
		// Validate deductions and bonuses cannot be negative
		if ((field === "deductions" || field === "bonuses") && parsedValue < 0) {
			parsedValue = 0;
		}

		setPayrollData((prev) => {
			const currentData = prev[empId] || {};
			const updatedData = {
				...currentData,
				[field]: parsedValue
			};
			...
			if (field === "deductions" || field === "bonuses") {
				const baseSalary =
					field === "baseSalary" ? parsedValue : updatedData.baseSalary || 0;
				const bonuses =
					field === "bonuses" ? parsedValue : updatedData.bonuses || 0;
				const deductions =
					field === "deductions" ? parsedValue : updatedData.deductions || 0;
				const maxDeductions = baseSalary + bonuses;

				if (deductions > maxDeductions) {
					updatedData.deductions = maxDeductions;
					alert(
						"Deductions cannot exceed base salary + bonuses. Setting to maximum allowed."
					);
				}
			}
	...
	const toggleTax = () => {
		...
		employees.forEach((emp) => {
			updated[emp.id] = {
				...payrollData[emp.id],
				deductions: newApplyTax ? emp.salary * 0.1 : 0
			};
		});
						...
						<TableBody>
							{records.map((r) => (
								<TableRow key={r.id}>
									...
									<TableCell>
										{r.status === "PENDING" && (
											<Button size="sm" onClick={() => markAsPaid(r.id)}>
